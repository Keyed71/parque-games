...
| Letters Round | React + HTML iframe | Any | `public/games/countdown.html` (id `countdown`); Countdown clone — 2–4 teams, 30/45/60s, manual “vowel/consonant” calling (official letter frequencies) or auto-deal, big clock with Web-Audio ticks/buzzer, optional Conundrum finale. Teacher adjudicates; the app reveals the best dictionary words and live-validates each team's word. Word list is bundled at `public/games/countdown-words.txt` (SOWPODS, UK-inclusive, 3–9 letters, ~388 KB gzipped) and fetched at runtime — no CSV |
| Word Battleships | React + HTML iframe + Firebase | A1–C1 | `public/games/battleships.html` (id `battleships`); cross-device twist on Battleships — 2 teams each secretly pick & place 4 words (3/4/4/5 letters) on phones, win by GUESSING all 4 of the other team's words (not sinking). Real-time multiplayer via **Firebase Realtime Database** (project `parque-battleships`, config embedded in the HTML — web SDK keys are public-safe). Page role comes from the URL: no params = projector/host (creates `rooms/{CODE}`, shows two per-team join QR codes); `?room=X&team=A\|B` = that team's phone. `?demo=1` = offline single-screen prototype (device-switcher bar, mock state) for solo testing. Words from `public/games/oxford-words.json` (Oxford 3000/5000 by CEFR, bucketed by length). Found letters orange, complete words green; side-by-side projector boards. **DB rules are open (`database.rules.json`) — set them in the Firebase console; classroom-scale, no real secrecy** |
| Alphabet Wheel | React + HTML iframe | A1–C2 | `public/games/alphabet-wheel.html` (id `alphabet-wheel`); 26 letters in a ring, theme in the centre. Teacher clicks a letter, types the class's word, it locks in that team's colour. 2–4 teams, 1/5/10 pts per letter, undo, keyboard shortcut (press a letter key), results screen with the full A–Z list. Themes come from the `Themes` tab (`theme_id, level, theme, hint, active`), wired into `THEME_CSV_URL` at the top of the HTML (`gid=1779773159`); falls back to a built-in list of ~50 themes if the sheet is unreachable. Starter themes in `alphabet-wheel-themes.csv` |
